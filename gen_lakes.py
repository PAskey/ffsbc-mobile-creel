...
The AssessEvent tab is the master list. This rewrites only the LAKES config and
DEFAULT_LAKE in index.html. Netlify's build step (scripts/build.sh) copies
index.html into site/ and stamps the service-worker cache on every deploy, so
you never edit site/ or sw.js yourself.

Workflow: edit the AssessEvent tab in SURVEY.xlsx -> run: python gen_lakes.py -> push.
Stocked species codes go comma-separated in the `comment` column (e.g. "EB, RB").
"""
...
def main():
    ...
    print(f"Updated index.html with {len(lakes)} lakes; default = {default} ({lakes[0][1]}).")
    print("Now push to GitHub - Netlify copies index.html into site/ and bumps the cache automatically.")
